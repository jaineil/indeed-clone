...
import { LOGIN_SUCCESS } from "../../_actions/actionTypes";
...
export function Login() {
	const { isAuth, isLoading, isError, errorMsg, user } = useSelector(
		(state) => state.login
	);
			...
			<Box className={classes.boxForm}>
				<Grid container spacing={3}>
					...
					<Grid style={{ margin: "10px 0" }} container item>
						...
						<Grid style={{ lineHeight: "0px" }} item>
							{" "}
							or
						</Grid>
						<Grid item>
	...
	) : user.userPersona === "JOB_SEEKER" ? (
		<Redirect to="/home" />
	) : user.userPersona === "EMPLOYER" ? (
		<Redirect to="/employer" />
	) : user.userPersona === "ADMIN" ? (
		<Redirect to="/admin" />
	) : (
		<Redirect to="/login" />
	);
